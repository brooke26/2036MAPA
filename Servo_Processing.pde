...
int xpos=180; // set x servo's value to mid point (0-180);
...
void draw()
{
  ...
  float invX = map(mouseX,0,width, width,0);
   float invY = map(mouseY,0,height, height,0);
  update(int(invX),int(invY));
}
